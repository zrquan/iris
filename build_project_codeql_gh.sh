...
entry=`grep "$cve" codeql_advisory_cves_mapped_w_commits.csv`
artifactId=`echo $entry | cut -d"," -f6`

if [ "${artifactId}" == "com.vaadin:vaadin-bom" ] || [ "${artifactId}" == "com.vaadin:vaadin-server" ] || [ "${artifactId}" == "com.vaadin:vaadin" ] || [ "${artifactId}" == "com.vaadin:vaadin-compatibility-server" ]; then 
    github_url="https://github.com/vaadin/framework"
elif [ "${artifactId}" == "com.vaadin:flow-server" ] || [ "${artifactId}" == "com.vaadin:flow-client" ]; then 
    github_url="https://github.com/vaadin/flow"
elif [ "${artifactId}" == "com.vaadin:checkbox-flow" ] || [ "${artifactId}" == "com.vaadin:vaadin-text-field-flow" ]; then
    github_url="https://github.com/vaadin/flow-components"
else
    github_url=`echo $entry | cut -d"," -f11`
fi

slug=`echo $github_url | rev | cut -d"/" -f1,2 | rev`
version=`echo $entry | cut -d"," -f13`
...
version_p=`echo $version | sed 's/\//~/'`
repo_dir="${nfull}_${cve}_${version_p}"
echo $repo_dir
